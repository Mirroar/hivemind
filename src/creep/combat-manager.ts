...
export default class CombatManager {
	...
	public manageFighting(creep: Creep) {
	...
	private healNearbyTargets(creep: Creep) {
		...
		const target = _.max(targets, c => Math.min(
			c.hitsMax - c.hits,
			healParts * (c.id === creep.id ? HEAL_POWER : RANGED_HEAL_POWER),
		));
	...
	public performKitingFighting(creep: Creep, target: AttackTarget) {
		...
		for (const enemy of enemyCreeps) {
			...
			for (const position of positions) {
				...
				if (distance > maxRange) {
					position.score -= (willFight ? 10 : -50) * (distance - maxRange);
					continue;
				}
	...
	public couldWinFightAgainst(creep: Creep, otherCreep: Creep): boolean {
		...
		if (creep.getActiveBodyparts(RANGED_ATTACK) * RANGED_ATTACK_POWER > otherCreep.getActiveBodyparts(HEAL) * HEAL_POWER * 2 + otherCreep.getActiveBodyparts(RANGED_ATTACK) * RANGED_ATTACK_POWER * 2) {
			return true;
		}
